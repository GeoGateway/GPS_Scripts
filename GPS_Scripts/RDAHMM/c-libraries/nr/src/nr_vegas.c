	...
	float *chi2a)
{
	...
	if (init <= 2) {
		...
		npg=NR_IMAX(ncall/k,2);
		calls=npg*k;
		dxg=1.0/ng;
		for (dv2g=1,i=1;i<=ndim;i++) dv2g *= dxg;
		dv2g=NR_SQR(calls*dv2g)/npg/npg/(npg-1.0);
		xnd=nd;
		dxg *= xnd;
		xjac=1.0/calls;
		for (j=1;j<=ndim;j++) {
		...
		if (nd != ndo) {
			...
			for (j=1;j<=ndim;j++) NR_rebin(ndo/xnd,nd,r,xin,xi[j]);
			ndo=nd;
		}
		if (nprn >= 0) {
			printf("%s:  ndim= %3d  ncall= %8.0f\n",
				" Input parameters for NR_vegas",ndim,calls);
	...
	for (it=1;it<=itmx;it++) {
		...
		for (;;) {
			...
			for (k=1;k<=npg;k++) {
				wgt=xjac;
				for (j=1;j<=ndim;j++) {
					xn=(kg[j]-NR_ran2(&idum))*dxg+1.0;
					ia[j]=NR_IMAX(NR_IMIN((int)(xn),NDMX),1);
					if (ia[j] > 1) {
