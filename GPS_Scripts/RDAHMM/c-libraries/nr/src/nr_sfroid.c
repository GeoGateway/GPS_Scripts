...
int main(void)  /* Program sfroid */
{
	...
	y=NR_matrix(1,NYJ,1,NYK);
	s=NR_matrix(1,NSI,1,NSJ);
	c=NR_f3tensor(1,NCI,1,NCJ,1,NCK);
	itmax=100;
	conv=5.0e-6;
	slowc=1.0;
	h=1.0/(M-1);
	...
	for (k=1;k<=(M-1);k++) {
		...
		y[1][k]=NR_plgndr(n,mm,x[k])*fac2;
		deriv = -((n-mm+1)*NR_plgndr(n+1,mm,x[k])-
			(n+1)*x[k]*NR_plgndr(n,mm,x[k]))/fac1;
	...
	for (;;) {
		...
		scanf("%f",&c2);
		if (c2 == 999) {
			NR_free_f3tensor(c,1,NCI,1,NCJ,1,NCK);
			NR_free_matrix(s,1,NSI,1,NSJ);
			NR_free_matrix(y,1,NYJ,1,NYK);
			return 0;
		}
		NR_solvde(itmax,conv,slowc,scalv,indexv,NE,NB,M,y,c,s);
