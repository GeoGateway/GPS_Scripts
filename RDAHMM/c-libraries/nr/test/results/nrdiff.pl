...
sub fuzzy_compare {
  ...
  # Compare each element
    ...
    if ($file1_element_i !~ /[-.dDeE0-9]+/ || $file2_element_i !~ /[-.dDeE0-9]+/) {
    ...
    # they're both numbers
      if ($file1_element_i == 0 && $file2_element_i == 0) {
      # these may be hex values, so check for that case and convert
        next if ($file1_element_i !~ /^0x/ && $file2_element_i !~ /^0x/);
        $file1_element_i = oct($file1_element_i);
        $file2_element_i = oct($file2_element_i);
