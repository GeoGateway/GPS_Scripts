	...
	unsigned long *jmax, float *prob)
{
	...
	*nout=0.5*ofac*hifac*n;
	...
	ndim=nfreq << 1;
	if (ndim > nwk) NR_error("workspaces too small in NR_fasper");
	...
	for (j=1;j<=n;j++) {
		...
		MOD(ckk,fndim)
		++ckk;
		NR_spread(y[j]-ave,wk1,ndim,ck,MACC);
		NR_spread(1.0,wk2,ndim,ckk,MACC);
	}
	NR_realft(wk1,ndim,1);
	NR_realft(wk2,ndim,1);
	df=1.0/(xdif*ofac);
	pmax = -1.0;
	for (k=3,j=1;j<=(*nout);j++,k+=2) {
		...
		cwt=sqrt(0.5+hc2wt);
		swt=NR_SIGN(sqrt(0.5-hc2wt),hs2wt);
		den=0.5*n+hc2wt*wk2[k]+hs2wt*wk2[k+1];
		cterm=NR_SQR(cwt*wk1[k]+swt*wk1[k+1])/den;
		sterm=NR_SQR(cwt*wk1[k+1]-swt*wk1[k])/(n-den);
		wk1[j]=j*df;
