	...
	unsigned long *lcd, int isign, arithcode *acode)
{
	...
	if (!isign) {
		...
		NR_error("NWK too small in NR_arcode.");
	} else {
		if (isign > 0) {
			if (*ich > acode->nch) NR_error("bad ich in NR_arcode.");
		}
		else {
		...
		acode->jdif=jh-jl;
		NR_arcsum(acode->ilob,acode->iupb,jh,NWK,acode->nrad,acode->nc);
		NR_arcsum(acode->ilob,acode->ilob,jl,NWK,acode->nrad,acode->nc);
			...
			if (*lcd > *lcode) {
				...
				*lcode *= 1.5;
				if ((*codep=(unsigned char *)realloc(*codep,
					...
					NR_error("Size expansion failed");
				}
			}
		...
		if (acode->nc-j < 1) NR_error("NWK too small in NR_arcode.");
		if (j) {
