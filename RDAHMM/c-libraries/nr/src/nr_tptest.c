...
void NR_tptest(float data1[], float data2[], unsigned long n, float *t,
	float *prob)
{
	...
	NR_avevar(data1,n,&ave1,&var1);
	NR_avevar(data2,n,&ave2,&var2);
	for (j=1;j<=n;j++)
		cov += (data1[j]-ave1)*(data2[j]-ave2);
	cov /= df=n-1;
	sd=sqrt((var1+var2-2.0*cov)/n);
	*t=(ave1-ave2)/sd;
	*prob=NR_betai(0.5*df,0.5,df/(df+(*t)*(*t)));
}
