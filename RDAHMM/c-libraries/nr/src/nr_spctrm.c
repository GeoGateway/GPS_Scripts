...
void NR_spctrm(FILE *fp, float p[], int m, int k, int ovrlap)
{
	...
	facm=m;
	facp=1.0/m;
	for (j=1;j<=mm;j++) sumw += NR_SQR(WINDOW(j,facm,facp));
	...
	for (kk=1;kk<=k;kk++) {
		...
		NR_four1(w1,mm,1);
		p[1] += (NR_SQR(w1[1])+NR_SQR(w1[2]));
		for (j=2;j<=m;j++) {
			j2=j+j;
			p[j] += (NR_SQR(w1[j2])+NR_SQR(w1[j2-1])
				+NR_SQR(w1[m44-j2])+NR_SQR(w1[m43-j2]));
		}
		den += sumw;
	}
	den *= m4;
	for (j=1;j<=m;j++) p[j] /= den;
	NR_free_vector(w2,1,m);
	NR_free_vector(w1,1,m4);
}
#undef WINDOW
#undef NRANSI
