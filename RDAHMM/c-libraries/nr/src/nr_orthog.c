...
void NR_orthog(int n, float anu[], float alpha[], float NR_beta[], float a[],
	float b[])
{
	int k,l;
	float **sig;
	...
	for (k=3;k<=n+1;k++) {
		...
		for (l=k;l<=looptmp;l++) {
			sig[k][l]=sig[k-1][l+1]+(alpha[l-1]-a[k-2])*sig[k-1][l]-
				b[k-2]*sig[k-2][l]+NR_beta[l-1]*sig[k-1][l-1];
		}
		a[k-1]=alpha[k-1]+sig[k][k+1]/sig[k][k]-sig[k-1][k]/sig[k-1][k-1];
		b[k-1]=sig[k][k]/sig[k-1][k-1];
	}
	NR_free_matrix(sig,1,2*n+1,1,2*n+1);
}
#undef NRANSI
