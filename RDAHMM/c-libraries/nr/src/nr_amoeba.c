	...
	float (*funk)(float []), int *nfunk)
{
	...
	*nfunk=0;
	...
	for (;;) {
		...
		if (*nfunk >= NMAX) NR_error("NMAX exceeded");
		*nfunk += 2;
		ytry=NR_amotry(p,y,psum,ndim,funk,ihi,-1.0);
		if (ytry <= y[ilo])
			ytry=NR_amotry(p,y,psum,ndim,funk,ihi,2.0);
		else if (ytry >= y[inhi]) {
			ysave=y[ihi];
			ytry=NR_amotry(p,y,psum,ndim,funk,ihi,0.5);
			if (ytry >= ysave) {
				for (i=1;i<=mpts;i++) {
					if (i != ilo) {
						for (j=1;j<=ndim;j++)
