...
void NR_stiff(float y[], float dydx[], int n, float *x, float htry, float eps,
	...
	void (*derivs)(float, float [], float []))
{
	...
	NR_jacobn(xsav,ysav,dfdx,dfdy,n);
	h=htry;
	for (jtry=1;jtry<=MAXTRY;jtry++) {
		...
		*x=xsav+A3X*h;
		...
		NR_lubksb(a,n,indx,g3);
		for (i=1;i<=n;i++)
			g4[i]=dydx[i]+h*C4X*dfdx[i]+(C41*g1[i]+C42*g2[i]+C43*g3[i])/h;
		NR_lubksb(a,n,indx,g4);
		for (i=1;i<=n;i++) {
			y[i]=ysav[i]+B1*g1[i]+B2*g2[i]+B3*g3[i]+B4*g4[i];
			err[i]=E1*g1[i]+E2*g2[i]+E3*g3[i]+E4*g4[i];
		}
		*x=xsav+h;
		if (*x == xsav) NR_error("stepsize not significant in NR_stiff");
		errmax=0.0;
		for (i=1;i<=n;i++) errmax=NR_FMAX(errmax,fabs(err[i]/yscal[i]));
		errmax /= eps;
		if (errmax <= 1.0) {
			*hdid=h;
			*hnext=(errmax > ERRCON ? SAFETY*h*pow(errmax,PGROW) : GROW*h);
			NR_free_vector(ysav,1,n);
			NR_free_vector(g4,1,n);
			NR_free_vector(g3,1,n);
			NR_free_vector(g2,1,n);
			NR_free_vector(g1,1,n);
			NR_free_vector(err,1,n);
			NR_free_vector(dysav,1,n);
			NR_free_matrix(dfdy,1,n,1,n);
			NR_free_vector(dfdx,1,n);
			NR_free_matrix(a,1,n,1,n);
			NR_free_ivector(indx,1,n);
			return;
		} else {
			*hnext=SAFETY*h*pow(errmax,PSHRNK);
			h=(h >= 0.0 ? NR_FMAX(*hnext,SHRNK*h) : NR_FMIN(*hnext,SHRNK*h));
		}
	}
	NR_error("exceeded MAXTRY in NR_stiff");
}
#undef SAFETY
#undef GROW
#undef PGROW
