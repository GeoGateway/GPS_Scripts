...
void NR_rkck(float y[], float dydx[], int n, float x, float h, float yout[],
	float yerr[], void (*derivs)(float, float [], float []))
{
	...
	NR_free_vector(ak6,1,n);
	NR_free_vector(ak5,1,n);
	NR_free_vector(ak4,1,n);
	NR_free_vector(ak3,1,n);
	NR_free_vector(ak2,1,n);
}
#undef NRANSI
