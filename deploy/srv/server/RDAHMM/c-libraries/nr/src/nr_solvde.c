...
void NR_solvde(int itmax, float conv, float slowc, float scalv[], int indexv[],
	int ne, int nb, int m, float **y, float ***c, float **s)
{
	...
	float err,erNR_rj,fac,vmax,vz,*ermax;

	kmax=NR_ivector(1,ne);
	ermax=NR_vector(1,ne);
	k1=1;
	k2=m;
	nvars=ne*m;
	j1=1;
	...
	for (it=1;it<=itmax;it++) {
		k=k1;
		NR_difeq(k,k1,k2,j9,ic3,ic4,indexv,ne,s,y);
		NR_pinvs(ic3,ic4,j5,j9,jc1,k1,c,s);
		for (k=k1+1;k<=k2;k++) {
			kp=k-1;
			NR_difeq(k,k1,k2,j9,ic1,ic4,indexv,ne,s,y);
			red(ic1,ic4,j1,j2,j3,j4,j9,ic3,jc1,jcf,kp,c,s);
			NR_pinvs(ic1,ic4,j3,j9,jc1,k,c,s);
		}
		k=k2+1;
		NR_difeq(k,k1,k2,j9,ic1,ic2,indexv,ne,s,y);
		red(ic1,ic2,j5,j6,j7,j8,j9,ic3,jc1,jcf,k2,c,s);
		NR_pinvs(ic1,ic2,j7,j9,jcf,k2+1,c,s);
		NR_bksub(ne,nb,jcf,k1,k2,c);
		err=0.0;
		for (j=1;j<=ne;j++) {
			jv=indexv[j];
			erNR_rj=vmax=0.0;
		...
		if (err < conv) {
			NR_free_vector(ermax,1,ne);
			NR_free_ivector(kmax,1,ne);
			return;
		}
	}
	NR_error("Too many iterations in NR_solvde");
}
#undef NRANSI
