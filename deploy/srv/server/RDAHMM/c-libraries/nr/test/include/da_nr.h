...
#ifndef lint
static char nr_hdr_rcsid[] = "$Id: da_nr.h,v 1.2 1996/09/24 17:21:27 granat Exp $";
#endif
/* 
 * $Log: da_nr.h,v $
 * Revision 1.2  1996/09/24 17:21:27  granat
 * Changed prototypes to include da_ n hacked files
 *
 * Revision 1.1  1996/09/24 15:55:42  granat
 * Initial revision
 *
...
void da_medfit(float x[], float y[], int ndata, float *a, float *b, float *abdev);
...
float da_select(unsigned long k, unsigned long n, float arr[]);
...
void da_svdcmp2(float **a, int m, int n, float w[], float **v, int maxiter);
void da_svdcmp3(float **a, int m, int n, float w[], float **v, int maxiter);
