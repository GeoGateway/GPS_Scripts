	...
	unsigned long n2, float *d, float *prob)
{
	...
	for (j=1;j<=n1;j++) {
		...
		NR_quadct(x1[j],y1[j],x2,y2,n2,&ga,&gb,&gc,&gd);
	...
	d2=0.0;
	for (j=1;j<=n2;j++) {
		NR_quadct(x2[j],y2[j],x1,y1,n1,&fa,&fb,&fc,&fd);
		NR_quadct(x2[j],y2[j],x2,y2,n2,&ga,&gb,&gc,&gd);
		d2=NR_FMAX(d2,fabs(fa-ga));
		d2=NR_FMAX(d2,fabs(fb-gb));
		d2=NR_FMAX(d2,fabs(fc-gc));
		d2=NR_FMAX(d2,fabs(fd-gd));
	}
	*d=0.5*(d1+d2);
	sqen=sqrt(n1*n2/(double)(n1+n2));
	NR_pearsn(x1,y1,n1,&r1,&dum,&dumm);
	NR_pearsn(x2,y2,n2,&r2,&dum,&dumm);
	rr=sqrt(1.0-0.5*(r1*r1+r2*r2));
	*prob=NR_probks(*d*sqen/(1.0+rr*(0.25-0.75/sqen)));
}
#undef NRANSI
