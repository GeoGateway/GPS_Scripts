...
void NR_bessik(float x, float xnu, float *ri, float *rk, float *rip, float *rkp)
{
	...
	if (x <= 0.0 || xnu < 0.0) NR_error("bad arguments in NR_bessik");
	nl=(int)(xnu+0.5);
	xmu=xnu-nl;
	xmu2=xmu*xmu;
	xi=1.0/x;
	...
	if (i > MAXIT) NR_error("x too large in NR_bessik; try asymptotic expansion");
	ril=FPMIN;
	ripl=h*ril;
	ril1=ril;
	rip1=ripl;
	...
	if (x < XMIN) {
		...
		if (i > MAXIT) NR_error("NR_bessk series failed to converge");
		rkmu=sum;
		rk1=sum1*xi2;
	} else {
		b=2.0*(1.0+x);
		...
		if (i > MAXIT) NR_error("NR_bessik: failure to converge in cf2");
		h=a1*h;
