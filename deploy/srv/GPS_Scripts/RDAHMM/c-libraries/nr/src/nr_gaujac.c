...
void NR_gaujac(float x[], float w[], int n, float alf, float bet)
{
	...
	for (i=1;i<=n;i++) {
		...
		if (its > MAXIT) NR_error("too many iterations in NR_gaujac");
		x[i]=z;
		w[i]=exp(NR_gammln(alf+n)+NR_gammln(bet+n)-NR_gammln(n+1.0)-
			NR_gammln(n+alfbet+1.0))*temp*pow(2.0,alfbet)/(pp*p2);
	}
}
#undef EPS
#undef MAXIT
